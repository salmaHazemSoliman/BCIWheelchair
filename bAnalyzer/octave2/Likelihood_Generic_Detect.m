function [DetectOut Debug] = Likelihood_Generic_Detect(DetectIn,likelihoodClass, directory, noiseFlag, idealFlag, butterFlag, NoneFilterFlag,f1FLag,f2FLag,f3FLag,f4FLag,f5FLag,f6FLag,LDAFLag,PCAFlag,CSP_LDAFlag,NoneFlag, preProjectedFlag)
...
%TODO according to the preprojectionflag, validate TrialData/VLDA/VPCA matrices
    if (preProjectedFlag == 0)
        ...
        if(idealFlag == 1)
         ...
         if(NoneFilterFlag == 1)
            if(f1FLag == 1)
                [Mu,Beta] = NoFilter(TrialData);
            elseif(f3FLag == 1)
                [Mu,temp] = NoFilter(TrialData, @min);
                [temp,Beta] = NoFilter(TrialData, @max);
            %TODO: support F3 to F6 flags
            endif
        endif
